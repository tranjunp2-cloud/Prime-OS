...
import { ArrowRight, BarChart3, Bot, CheckCircle2, GripVertical, Layers3, Rocket, ScanSearch, ShieldCheck, Sparkles } from 'lucide-react';
    ...
    <div className="space-y-6 border-b pb-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-3 font-display text-4xl font-semibold tracking-tight md:text-5xl">Consulting Agent</h1>
          <p className="mt-3 max-w-3xl text-base text-muted-foreground">One product for KPI dashboarding, signal intelligence, and launch decisions. Reads evidence across PrimeOS; source systems keep ownership.</p>
        </div>
        <div className="rounded-2xl border bg-card p-4 text-sm text-muted-foreground">
          <div className="font-semibold text-foreground">Boundary protected</div>
          Board moves create triage/audit state only — no Demand/COS/Finance mutation.
        </div>
      </div>
      <div className="flex max-w-full gap-2 overflow-x-auto rounded-2xl bg-muted p-1">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          return (
            <button key={tab.id} type="button" onClick={() => onChangeTab(tab.id)} className={cn('min-w-56 rounded-xl px-4 py-3 text-left transition-colors', activeTab === tab.id ? 'bg-card text-primary shadow-sm' : 'text-muted-foreground hover:bg-card/60')}>
              <div className="flex items-center gap-2 font-semibold"><Icon className="size-4" />{tab.label}</div>
              <div className="mt-1 text-xs text-muted-foreground">{tab.copy}</div>
            </button>
          );
        })}
      </div>
    ...
    <div className="space-y-5 text-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <DialogTitle className="mt-2 text-2xl">{card.title}</DialogTitle>
          <DialogDescription className="mt-2">{card.detail}</DialogDescription>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-4">
        <SummaryMetricCard label="Packages" value={metrics.packages} meta="Consulting decision packages." icon={<Bot className="size-5" />} tone="purple" />
        <SummaryMetricCard label="Ready" value={metrics.readyForDemand} meta="Ready for owner handoff." icon={<CheckCircle2 className="size-5" />} tone="success" />
        <SummaryMetricCard label="Blocked" value={metrics.blocked} meta="Guardrail protected." icon={<ShieldCheck className="size-5" />} tone="orange" />
        <SummaryMetricCard label="Moves" value={audit.length} meta="Local KPI board audit." icon={<Layers3 className="size-5" />} tone="info" />
      </div>
            ...
            return (
              <div key={lane.id} className="min-h-[420px] rounded-2xl border bg-card/80 p-3" onDragOver={(event) => event.preventDefault()} onDrop={(event) => moveCard(event.dataTransfer.getData('text/plain'), lane.id)}>
                <div className="rounded-xl border bg-background/90 p-3">
                  <div className="flex items-center justify-between gap-3"><div><h3 className="font-semibold">{lane.label}</h3><p className="text-xs text-muted-foreground">{lane.detail}</p></div><Badge variant="outline">{laneCards.length}</Badge></div>
                </div>
                <div className="mt-3 space-y-3">
                  {laneCards.map((card) => <div key={card.id} draggable onClick={() => setSelectedCardId(card.id)} onDragStart={(event) => event.dataTransfer.setData('text/plain', card.id)} className={cn('cursor-pointer rounded-xl border bg-background p-4 text-left shadow-sm transition hover:border-primary/50', selectedCardId === card.id && 'border-primary ring-1 ring-primary/30')}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="truncate font-semibold">{card.title}</div><div className={cn('mt-3 inline-flex rounded-xl border px-3 py-1 text-2xl font-bold', toneClass(card.tone))}>{card.value}</div></div><GripVertical className="size-4 shrink-0 text-muted-foreground" /></div><p className="mt-3 text-sm text-muted-foreground">{card.detail}</p><Button type="button" size="sm" variant="outline" className="mt-3 w-full" onClick={(event) => { event.stopPropagation(); setSelectedCardId(card.id); setDetailCardId(card.id); }}>Open details</Button></div>)}
