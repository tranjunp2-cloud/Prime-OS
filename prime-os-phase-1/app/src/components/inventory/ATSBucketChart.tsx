...
import { computeATS } from '@/lib/ats-calculations';
...
export function ATSBucketChart({ warehouseId, topN = 15 }: ATSBucketChartProps) {
  ...
  const aggregated = positions
    ...
    .reduce<Record<string, { skuId: string; name: string; skuCode: string; on_hand: number; reserved: number; inbound: number; unfulfillable: number; ats: number }>>((acc, p) => {
      ...
      if (!acc[p.sku_id]) {
        acc[p.sku_id] = {
          ...
          on_hand: 0, reserved: 0, inbound: 0, unfulfillable: 0, ats: 0,
        };
      }
      ...
      const result = computeATS(
        acc[p.sku_id].on_hand,
        acc[p.sku_id].reserved,
  ...
  if (chartData.length === 0) {
    return (
      <Card>
        <CardHeader><CardTitle className="text-sm">ATS Breakdown by SKU</CardTitle></CardHeader>
      ...
      <CardContent>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
            ...
            <Bar dataKey="Reserved" fill="#8b5cf6" radius={[2, 2, 0, 0]} maxBarSize={28} />
