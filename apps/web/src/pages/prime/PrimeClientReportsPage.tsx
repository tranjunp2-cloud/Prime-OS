...
const metrics: Array<{ key: MetricKey; label: string; value: string; detail: string; description: string; formula: string; icon: typeof BarChart3 }> = [
  { key: 'revenue', label: 'Total Revenue', value: '₫4.82B', detail: '+18.4% vs previous period', description: 'Gross order revenue generated by all selected stores and channels during the applied date range.', formula: 'Sum of gross order value before refunds and selling fees', icon: CircleDollarSign },
  { key: 'orders', label: 'Total Orders', value: '12,480', detail: '+12.6% vs previous period', description: 'Number of unique orders created across the selected stores, including every order status.', formula: 'Count of unique order IDs created in the period', icon: ShoppingBag },
  { key: 'products', label: 'Total Products Sold', value: '18,942', detail: '+9.2% vs previous period', description: 'Total quantity of product units sold. One order may contribute multiple units.', formula: 'Sum of fulfilled order-line quantities', icon: PackageCheck },
  { key: 'retention', label: 'Repeat Purchase Rate', value: '38.6%', detail: '+4.1 pts vs previous period', description: 'Share of purchasing customers who had already completed at least one earlier order.', formula: 'Repeat customers ÷ total purchasing customers × 100', icon: UsersRound },
  { key: 'shipping', label: 'Avg Shipping Cost', value: '₫31,400', detail: '-5.3% vs previous period', description: 'Average shipping cost charged by carriers for orders shipped during the selected period.', formula: 'Total carrier shipping cost ÷ shipped orders', icon: Truck },
];
...
const distributions = [
  { title: 'Order Cancellation Reasons', subtitle: 'Reasons & initiators', description: 'Distribution of canceled orders by the primary reason or party that initiated the cancellation.', formula: 'Orders in each reason ÷ all canceled orders × 100', scope: 'Only orders canceled during the applied date range are included.', items: [{ label: 'Customer request', value: 38, color: '#635bff' }, { label: 'Out of stock', value: 27, color: '#818cf8' }, { label: 'Payment failed', value: 21, color: '#38bdf8' }, { label: 'Seller initiated', value: 14, color: '#cbd5e1' }] },
  { title: 'Top 5 Regions by Revenue', subtitle: 'Province contribution', description: 'Share of gross revenue generated by the five highest-performing destination provinces.', formula: 'Region gross revenue ÷ top-five region revenue × 100', scope: 'The percentages are normalized to these five regions and do not represent every province.', items: [{ label: 'Ho Chi Minh City', value: 31, color: '#635bff' }, { label: 'Hanoi', value: 26, color: '#818cf8' }, { label: 'Da Nang', value: 17, color: '#38bdf8' }, { label: 'Binh Duong', value: 14, color: '#10b981' }, { label: 'Dong Nai', value: 12, color: '#f59e0b' }] },
  { title: 'Shipping Carrier Breakdown', subtitle: 'Delivered order share', description: 'Distribution of successfully delivered orders by the carrier that completed delivery.', formula: 'Delivered orders per carrier ÷ all delivered orders × 100', scope: 'Canceled, returned, and in-transit orders are excluded.', items: [{ label: 'GHN', value: 44, color: '#635bff' }, { label: 'GHTK', value: 34, color: '#38bdf8' }, { label: 'ViettelPost', value: 22, color: '#10b981' }] },
];
...
const storeMetricHelp: Record<string, { description: string; formula?: string }> = {
  'Gross Revenue': {
    description: 'Total value of orders placed during the selected period before discounts, refunds, and selling fees are deducted.',
    formula: 'Sum of order value before deductions',
  },
  'Net Revenue': {
    description: 'Revenue retained after discounts, refunds, cancellations, marketplace fees, and payment fees are deducted.',
    formula: 'Gross revenue − deductions',
  },
  'Avg Order Value': {
    description: 'The average gross value generated by each order during the selected period.',
    formula: 'Gross revenue ÷ total orders',
  },
  'Total Customers': {
    description: 'Number of unique customers who placed at least one order through this store or channel during the selected period.',
  },
};

function MetricColumnHeader({ column }: { column: string }) {
  const help = storeMetricHelp[column];
  if (!help) return <>{column}</>;
    ...
    <span className="inline-flex items-center gap-1.5">
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <button type="button" className="grid size-8 shrink-0 place-items-center rounded-md text-slate-400 transition-colors hover:bg-slate-200/70 hover:text-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary dark:hover:bg-zinc-700 dark:hover:text-zinc-100" aria-label={`Explain ${column}`}>
            <Info className="size-3.5" aria-hidden="true" />
          </button>
        </TooltipTrigger>
        <TooltipContent side="top" align="start" className="max-w-72 p-3 normal-case tracking-normal">
          <p className="font-semibold">{column}</p>
          <p className="mt-1 text-xs leading-5 text-popover-foreground/80">{help.description}</p>
          {help.formula ? <p className="mt-2 rounded bg-muted px-2 py-1 text-[11px] font-medium text-muted-foreground">Formula: {help.formula}</p> : null}
        </TooltipContent>
      </Tooltip>
    </span>
  );
...
function DistributionHelp({ card }: { card: typeof distributions[number] }) {
  return <Tooltip><TooltipTrigger asChild><button type="button" className="grid size-8 shrink-0 place-items-center rounded-md text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label={`Explain ${card.title}`}><Info className="size-4" aria-hidden="true" /></button></TooltipTrigger><TooltipContent side="top" align="end" className="max-w-80 p-3"><p className="font-semibold">{card.title}</p><p className="mt-1 text-xs leading-5 text-popover-foreground/80">{card.description}</p><p className="mt-2 rounded bg-muted px-2 py-1 text-[11px] font-medium text-muted-foreground">Formula: {card.formula}</p><p className="mt-2 text-[11px] leading-4 text-popover-foreground/70">{card.scope}</p></TooltipContent></Tooltip>;
}
...
export function PrimeClientReportsPage() {
  ...
  return <div className="space-y-6 p-4 pb-28 md:p-6">
    ...
    <TooltipProvider delayDuration={150}><section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5" aria-label="Analytics KPI summary">{metrics.map((metric) => { const Icon = metric.icon; const active = activeMetric === metric.key; return <article key={metric.key} className={cn('relative overflow-hidden rounded-xl border bg-white shadow-sm transition-all hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-md', active ? 'border-primary ring-1 ring-primary/20' : 'border-slate-200')}><button type="button" onClick={() => setActiveMetric(metric.key)} aria-pressed={active} aria-label={`Show ${metric.label} trend`} className="w-full p-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary"><span className={cn('grid size-9 place-items-center rounded-lg', active ? 'bg-primary text-white' : 'bg-indigo-50 text-indigo-600')}><Icon className="size-4" /></span><div className="mt-4 pr-7 text-xs font-semibold text-slate-500">{metric.label}</div><div className="mt-1 text-2xl font-semibold tabular-nums text-slate-900">{metric.value}</div><div className={cn('mt-1 text-xs font-semibold', metric.key === 'shipping' ? 'text-emerald-600' : 'text-indigo-600')}>{metric.detail}</div></button><Tooltip><TooltipTrigger asChild><button type="button" className="absolute right-3 top-3 grid size-8 place-items-center rounded-md text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label={`Explain ${metric.label}`}><Info className="size-4" aria-hidden="true" /></button></TooltipTrigger><TooltipContent side="top" align="end" className="max-w-72 p-3"><p className="font-semibold">{metric.label}</p><p className="mt-1 text-xs leading-5 text-popover-foreground/80">{metric.description}</p><p className="mt-2 rounded bg-muted px-2 py-1 text-[11px] font-medium text-muted-foreground">Formula: {metric.formula}</p><p className="mt-2 text-[11px] leading-4 text-popover-foreground/70">The change compares this value with the immediately preceding period of equal length.</p></TooltipContent></Tooltip></article>; })}</section></TooltipProvider>
