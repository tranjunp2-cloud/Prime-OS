...
import { AlertCircle, CircleDot, Copy, DollarSign, Download, ExternalLink, MoreHorizontal, Pencil, Plus, Search, Send, ShoppingBag, SlidersHorizontal, Store, Trash2, Unplug, X } from 'lucide-react';
...
export default function Products() {
  ...
  const filtered = useMemo(() => rows.filter(({ product, matrix }) => {
    ...
    return matchesSearch && matchesSync && matchesView && matchesChannel && matchesCategory && matchesVariants && matchesLegend;
  }), [catalogView, categoryFilter, channelFilter, legendFilter, rows, search, syncFilter, variantFilter]);
  ...
  return <div className="space-y-5 p-4 md:p-6">
    <WorkspacePageHeader title="Master Product Catalog" description="Manage product master data once and publish consistently across every sales channel." icon={ShoppingBag} actions={<Button onClick={() => navigate('/products/new')}><Plus className="size-4" />Add Master Product</Button>} />
    ...
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
      <nav className="flex gap-1 overflow-x-auto border-b border-slate-200 px-3" aria-label="Catalog status">
        {([['all', 'All'], ['active', 'Active'], ['unlisted', 'Unlisted'], ['other', 'Other']] as Array<[CatalogView, string]>).map(([key, label]) => <button key={key} type="button" onClick={() => setCatalogView(key)} className={cn('relative min-h-11 shrink-0 px-3 text-sm font-semibold transition-colors', catalogView === key ? 'text-indigo-700 after:absolute after:inset-x-2 after:bottom-0 after:h-0.5 after:bg-indigo-600' : 'text-slate-500 hover:text-slate-900')}>{label}<span className="ml-1.5 text-xs tabular-nums text-slate-400">{catalogCounts[key]}</span></button>)}
      </nav>
