...
export default function Docs() {
  ...
  return <div className="space-y-6 p-4 pb-20 text-foreground md:p-6">
    ...
    <section className="rounded-xl border border-border bg-gradient-to-br from-indigo-50 to-white p-5 dark:from-indigo-950/35 dark:to-card md:p-7">
      <div className="mx-auto max-w-3xl text-center"><p className="text-xs font-semibold uppercase tracking-wider text-indigo-700 dark:text-indigo-300">Product documentation</p><h2 className="mt-2 text-2xl font-bold text-foreground">Learn how Prime OS works</h2><p className="mt-2 text-sm text-muted-foreground">Explore concepts, end-to-end workflows, and configuration guides.</p><div className="relative mt-5"><Search className="pointer-events-none absolute left-4 top-1/2 size-5 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search concepts, workflows, or configuration..." aria-label="Search documentation" className="h-12 bg-background pl-12 text-base shadow-sm" /></div></div>
    </section>

    <section><div className="flex items-center justify-between gap-4"><div><h2 className="text-lg font-semibold text-foreground">Browse by category</h2><p className="mt-1 text-sm text-muted-foreground">Choose an operating domain to narrow the documentation.</p></div><span className="text-xs font-medium tabular-nums text-muted-foreground">{articles.length} articles</span></div><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{categories.map((item) => { const Icon = categoryMeta[item].icon; const count = articles.filter((article) => article.category === item).length; const active = category === item; return <button key={item} type="button" onClick={() => setCategory(active ? 'All' : item)} aria-pressed={active} className={cn('flex min-h-24 items-center gap-3 rounded-xl border bg-card p-4 text-left transition-colors hover:border-indigo-300 hover:bg-indigo-50/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 dark:hover:border-indigo-700 dark:hover:bg-indigo-950/25', active ? 'border-indigo-500 bg-indigo-50 ring-1 ring-indigo-100 dark:bg-indigo-950/40 dark:ring-indigo-900' : 'border-border')}><span className={cn('grid size-10 shrink-0 place-items-center rounded-lg', active ? 'bg-indigo-600 text-white' : 'bg-indigo-50 text-indigo-700 dark:bg-indigo-950/60 dark:text-indigo-300')}><Icon className="size-5" /></span><span className="min-w-0"><span className="block text-sm font-semibold text-foreground">{item}</span><span className="mt-1 block text-xs text-muted-foreground">{categoryMeta[item].description}</span><span className="mt-1 block text-xs font-medium text-indigo-700 dark:text-indigo-300">{count} article{count === 1 ? '' : 's'}</span></span></button>; })}</div></section>

    <section><div className="flex items-end justify-between gap-4"><div><h2 className="text-lg font-semibold text-foreground">{category === 'All' ? 'Recommended articles' : category}</h2><p className="mt-1 text-sm text-muted-foreground">{filtered.length} result{filtered.length === 1 ? '' : 's'} matching the current view.</p></div>{category !== 'All' && <Button variant="ghost" size="sm" onClick={() => setCategory('All')}>Clear category</Button>}</div><div className="mt-4 overflow-hidden rounded-xl border border-border bg-card"><div className="divide-y divide-border">{filtered.map((article) => <button key={article.id} type="button" onClick={() => openArticle(article)} className="flex min-h-24 w-full items-center gap-4 p-4 text-left transition-colors hover:bg-muted/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-indigo-500"><span className="grid size-10 shrink-0 place-items-center rounded-lg bg-muted text-muted-foreground"><FileText className="size-5" /></span><span className="min-w-0 flex-1"><span className="flex flex-wrap items-center gap-2"><span className="text-sm font-semibold text-foreground">{article.title}</span>{article.popular && <span className="rounded bg-indigo-50 px-1.5 py-0.5 text-[10px] font-semibold text-indigo-700 dark:bg-indigo-950/60 dark:text-indigo-300">Popular</span>}</span><span className="mt-1 block text-sm text-muted-foreground">{article.summary}</span><span className="mt-2 flex flex-wrap items-center gap-3 text-xs text-muted-foreground/80"><span>{article.category}</span><span className="inline-flex items-center gap-1"><Clock3 className="size-3" />{article.readTime} min read</span><span>Updated {article.updated}</span></span></span><ArrowRight className="size-4 shrink-0 text-muted-foreground" /></button>)}{filtered.length === 0 && <div className="grid min-h-48 place-items-center p-6 text-center"><div><Search className="mx-auto size-6 text-muted-foreground" /><p className="mt-3 text-sm font-semibold text-foreground">No documentation found</p><p className="mt-1 text-xs text-muted-foreground">Try a different keyword or clear the category filter.</p></div></div>}</div></div></section>
...
function ArticleDrawer({ article, onClose }: { article: DocArticle | null; onClose: () => void }) {
  return <Sheet open={Boolean(article)} onOpenChange={(open) => !open && onClose()}><SheetContent className="w-full overflow-y-auto p-0 sm:max-w-2xl">{article && <><SheetHeader className="border-b border-border p-5"><p className="text-xs font-semibold uppercase tracking-wider text-indigo-700 dark:text-indigo-300">{article.category}</p><SheetTitle className="text-left text-xl">{article.title}</SheetTitle><SheetDescription className="text-left">{article.summary}</SheetDescription><div className="flex gap-3 pt-1 text-xs text-muted-foreground"><span>{article.readTime} min read</span><span>Updated {article.updated}</span></div></SheetHeader><article className="space-y-7 p-5">{article.sections.map((section) => <section key={section.heading}><h2 className="text-base font-semibold text-foreground">{section.heading}</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">{section.body}</p>{section.steps && <ol className="mt-3 space-y-2">{section.steps.map((step, index) => <li key={step} className="flex gap-3 text-sm text-foreground/90"><span className="grid size-6 shrink-0 place-items-center rounded-full bg-indigo-50 text-xs font-semibold text-indigo-700 dark:bg-indigo-950/60 dark:text-indigo-300">{index + 1}</span><span className="pt-0.5">{step}</span></li>)}</ol>}</section>)}{article.helpSlug ? <aside className="rounded-xl border border-amber-300/70 bg-amber-50/70 p-4 dark:border-amber-500/30 dark:bg-amber-500/10"><p className="text-xs font-semibold uppercase tracking-wider text-amber-700 dark:text-amber-300">Having trouble?</p><p className="mt-1 text-sm text-muted-foreground">Open the matching troubleshooting article for symptoms, recovery steps, and escalation options.</p><Button asChild variant="outline" className="mt-3 bg-background"><Link to={`/faq/${article.helpSlug}?from_workspace=main`}>View troubleshooting<ArrowRight className="size-4" /></Link></Button></aside> : null}</article></>}</SheetContent></Sheet>;
}
