...
const stockRows = productOptions.flatMap((product) => Object.entries(product.inventory ?? {}).map(([warehouseId, quantity], index) => {
...
export default function Warehouses() {
  ...
  const filteredStock = useMemo(() => stockRows.filter((row) => (warehouseFilter === 'all' || row.warehouseId === warehouseFilter) && (!search || `${row.sku} ${row.product} ${row.warehouse}`.toLowerCase().includes(search.toLowerCase()))), [search, warehouseFilter]);
  const header = {
    mapping: {
  ...
  return <TooltipProvider delayDuration={150}><div className="space-y-5 p-4 md:p-6">
    ...
    <OperationDrawer type={operation} warehouses={warehouses} products={productOptions} onClose={() => setOperation(null)} onSubmit={(title, description) => { setOperation(null); toast({ title, description }); }} />
  </div></TooltipProvider>;
}
...
function StockLevels({ search, setSearch, warehouseFilter, setWarehouseFilter, warehouses, rows, onOpenWarehouse }: { search: string; setSearch: (value: string) => void; warehouseFilter: string; setWarehouseFilter: (value: string) => void; warehouses: PhysicalWarehouse[]; rows: typeof stockRows; onOpenWarehouse: (name: string) => void }) {
...
function OperationDrawer({ type, warehouses, products, onClose, onSubmit }: { type: OperationType; warehouses: PhysicalWarehouse[]; products: typeof productOptions; onClose: () => void; onSubmit: (title: string, description: string) => void }) {
