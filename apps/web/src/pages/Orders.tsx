...
const statuses: Array<{ key: StatusFilter; label: string; count: number }> = [
  ...
  { key: 'pending', label: 'Pending Confirmation', count: 124 },
  { key: 'ready', label: 'Awaiting Shipment', count: 45 },
  { key: 'shipping', label: 'In Transit', count: 326 },
...
function OrderStatus({ status }: { status: string }) {
  ...
  return <span className={cn('inline-flex rounded-md border px-2 py-1 text-xs font-semibold', tone)}>{status}</span>;
}
...
export default function Orders({ view = 'all' }: { view?: OrderView }) {
  ...
  const pageCopy = view === 'fulfillment'
    ? { title: 'Fulfillment & Shipping', description: 'Track picking, packing, carrier handoff, and delivery status across every sales channel.', icon: Truck }
    : view === 'returns'
      ? { title: 'Returns & Exchanges', description: 'Manage returns, exchanges, and refunds originating from Web, POS, and marketplaces.', icon: PackageCheck }
      : { title: 'All Orders', description: 'Monitor revenue, unblock fulfillment, and manage every order across stores and sales channels.', icon: ShoppingBag };
  ...
  return <div className="space-y-5 p-4 pb-28 md:p-6">
    <WorkspacePageHeader title={pageCopy.title} description={pageCopy.description} icon={pageCopy.icon} />

    <nav className="flex gap-1 overflow-x-auto border-b border-slate-200" aria-label="Order lifecycle">
      {statuses.map((item) => <button key={item.key} type="button" onClick={() => setStatus(item.key)} className={cn('relative min-h-11 shrink-0 px-3 text-sm font-semibold transition-colors', status === item.key ? 'text-indigo-700 after:absolute after:inset-x-2 after:bottom-0 after:h-0.5 after:bg-indigo-600' : 'text-slate-500 hover:text-slate-900')}>{item.label}<span className={cn('ml-1.5 text-xs tabular-nums', status === item.key ? 'text-indigo-500' : 'text-slate-400')}>{item.count}</span></button>)}
    </nav>

    <ChannelSourceFilter value={channel} onChange={setChannel} counts={channelCounts} />

    <div className={cn('grid items-start gap-4 transition-[grid-template-columns] lg:grid-cols-[280px_minmax(0,1fr)]', queueCollapsed && 'lg:grid-cols-[56px_minmax(0,1fr)]')}>
      <aside className="overflow-hidden rounded-xl border border-slate-200 bg-white lg:sticky lg:top-4">
        {queueCollapsed ? <button type="button" onClick={() => setQueueCollapsed(false)} className="hidden min-h-[320px] w-full flex-col items-center gap-3 py-4 text-slate-500 transition-colors hover:bg-slate-50 hover:text-slate-900 lg:flex" aria-label="Expand urgent action queue"><ChevronRight className="size-4" /><span className="[writing-mode:vertical-rl] text-xs font-semibold tracking-wider">URGENT QUEUE · 23</span></button> : <>
          <div className="flex items-center border-b border-slate-200 px-4 py-3"><div><h2 className="text-xs font-semibold tracking-wider text-slate-500">WORK QUEUE</h2><p className="mt-1 text-xs text-slate-400">Saved views and blockers</p></div><button type="button" onClick={() => setQueueCollapsed(true)} className="ml-auto hidden size-9 place-items-center rounded-md text-slate-400 hover:bg-slate-100 hover:text-slate-700 lg:grid" aria-label="Collapse work queue"><ChevronLeft className="size-4" /></button></div>
          <div className="border-b border-slate-200 p-2"><p className="px-2 pb-1 pt-2 text-[10px] font-semibold uppercase tracking-wider text-slate-400">Saved views</p>{savedViews.map((view) => <button key={view.label} type="button" onClick={() => activateView(view.label)} className={cn('flex min-h-10 w-full items-center gap-2 rounded-md px-2 text-left text-sm transition-colors', activeView === view.label ? 'bg-indigo-50 font-semibold text-indigo-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900')}><Bookmark className="size-3.5" /><span className="flex-1">{view.label}</span><span className="text-xs tabular-nums text-slate-400">{view.count}</span></button>)}</div>
          <div className="space-y-2 p-3"><p className="px-1 text-[10px] font-semibold uppercase tracking-wider text-slate-400">Urgent blockers</p>{urgentActions.map((item) => <article key={item.title} className="rounded-lg border border-slate-200 p-3 transition-colors hover:border-slate-300 hover:bg-slate-50/60"><span className={cn('inline-flex rounded-md px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide', item.priority === 'High' ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-700')}>{item.priority}</span><h3 className="mt-2 text-sm font-semibold text-slate-900">{item.title}</h3><p className="mt-1 text-xs leading-5 text-slate-500">{item.detail}</p><button type="button" onClick={() => runUrgentAction(item.action, item.view)} className="mt-2 min-h-9 text-xs font-semibold text-indigo-700 hover:text-indigo-900">{item.action} &gt;</button></article>)}</div>
        </>}
      </aside>
      ...
      <div className="min-w-0 space-y-4">
        <div className="flex flex-col gap-2 md:flex-row md:items-center">
          ...
          <div className="grid shrink-0 grid-cols-2 gap-2 md:flex md:items-center">
            <button type="button" onClick={() => setUnpaidOnly((value) => !value)} className={cn('inline-flex h-10 items-center justify-center whitespace-nowrap rounded-lg border px-3 text-sm font-semibold transition-colors', unpaidOnly ? 'border-amber-300 bg-amber-50 text-amber-700' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50')}><AlertTriangle className="mr-1.5 size-4" />Unpaid</button>
            <Button variant="outline" onClick={() => setFiltersOpen(true)} className="h-10 whitespace-nowrap border-slate-200 px-3 text-slate-700"><Filter className="size-4" />Advanced Filters{selectedStore !== 'All Stores' ? <span className="ml-1 grid size-5 place-items-center rounded-full bg-indigo-100 text-[10px] font-bold text-indigo-700">1</span> : null}</Button>
          </div>
        </div>
        {activeView || status !== 'all' || unpaidOnly || selectedStore !== 'All Stores' ? <div className="flex flex-wrap items-center gap-2 rounded-lg border border-slate-200 bg-slate-50/60 px-3 py-2"><span className="mr-1 text-xs font-semibold text-slate-500">Active filters</span>{activeView ? <button type="button" onClick={() => setActiveView(null)} className="inline-flex min-h-8 items-center gap-1.5 rounded-md border border-indigo-200 bg-white px-2.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-50">{activeView}<X className="size-3" /></button> : null}{status !== 'all' ? <button type="button" onClick={() => setStatus('all')} className="inline-flex min-h-8 items-center gap-1.5 rounded-md border border-slate-200 bg-white px-2.5 text-xs font-semibold text-slate-700 hover:bg-slate-100">Status: {statuses.find((item) => item.key === status)?.label}<X className="size-3" /></button> : null}{selectedStore !== 'All Stores' ? <button type="button" onClick={() => setSelectedStore('All Stores')} className="inline-flex min-h-8 items-center gap-1.5 rounded-md border border-slate-200 bg-white px-2.5 text-xs font-semibold text-slate-700 hover:bg-slate-100">Store: {selectedStore}<X className="size-3" /></button> : null}{unpaidOnly ? <button type="button" onClick={() => setUnpaidOnly(false)} className="inline-flex min-h-8 items-center gap-1.5 rounded-md border border-amber-200 bg-white px-2.5 text-xs font-semibold text-amber-700 hover:bg-amber-50">Unpaid<X className="size-3" /></button> : null}<span className="ml-auto text-xs tabular-nums text-slate-500">{filtered.length} demo orders</span><button type="button" onClick={() => { setActiveView(null); setStatus('all'); setSelectedStore('All Stores'); setUnpaidOnly(false); }} className="min-h-8 text-xs font-semibold text-slate-600 hover:text-slate-900">Clear all</button></div> : null}
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white"><div className="overflow-x-auto"><table className="w-full min-w-[1240px] text-left"><thead className="border-b border-slate-200 bg-slate-50/60"><tr><th className="w-12 px-4 py-3"><Checkbox checked={selected.length === filtered.length && filtered.length > 0} onCheckedChange={() => setSelected(selected.length === filtered.length ? [] : filtered.map((item) => item.id))} aria-label="Select all orders" /></th>{['ORDER ID', 'CUSTOMER / CRM', 'STORE / CHANNEL', 'FULFILLMENT WAREHOUSE', 'SLA', 'TOTAL AMOUNT', 'PAYMENT', 'STATUS'].map((header) => <th key={header} className={cn('px-4 py-3 text-xs font-semibold uppercase tracking-wider text-slate-500', header === 'TOTAL AMOUNT' && 'text-right')}>{header}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{filtered.map((order, orderIndex) => <tr key={order.id} onClick={() => setDetail(order)} className="cursor-pointer transition-colors hover:bg-slate-50/70"><td className="px-4 py-3" onClick={(event) => event.stopPropagation()}><Checkbox checked={selected.includes(order.id)} onCheckedChange={() => toggleOrder(order.id)} aria-label={`Select ${order.id}`} /></td><td className="px-4 py-3"><div className="flex items-center gap-3"><div className="flex -space-x-1.5">{[productImages[orderIndex % productImages.length], productImages[(orderIndex + 1) % productImages.length]].map((image, imageIndex) => <img key={image} src={image} alt="" className="size-9 rounded-md border border-slate-200 object-cover" style={{ zIndex: 2 - imageIndex }} />)}</div><div><button type="button" onClick={() => setDetail(order)} className="font-semibold text-slate-900 hover:text-primary">{order.id}</button><div className="mt-1 text-xs text-slate-500">{order.date}</div></div></div></td><td className="px-4 py-3"><div className="flex items-center gap-2 font-medium text-slate-900">{order.customer}{order.tags.map((tag) => <span key={tag} title={tag} className="inline-flex items-center gap-1 text-[10px] font-semibold text-rose-600"><span className="size-1.5 rounded-full bg-rose-400" />{tag}</span>)}</div><div className="mt-1 flex items-center gap-1 text-xs text-slate-500"><UserRound className="size-3" />{order.assignee}</div></td><td className="px-4 py-3"><StoreIdentity store={order.store} owner={order.owner} />{order.syncError ? <div className="mt-1 flex items-center gap-1 text-xs font-semibold text-rose-600"><AlertTriangle className="size-3" />Sync retry required</div> : null}</td><td className="px-4 py-3"><span className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-700"><Warehouse className="size-4 text-slate-400" />{order.warehouse}</span><div className={cn('mt-1 text-xs font-medium', order.reservation === 'Allocation failed' ? 'text-rose-600' : 'text-slate-500')}>{order.reservation}</div></td><td className="px-4 py-3"><span className={cn('inline-flex items-center gap-1.5 text-xs font-semibold', order.slaRisk ? 'text-rose-600' : 'text-slate-500')}><Clock3 className="size-3.5" />{order.sla}</span></td><td className="px-4 py-3 text-right font-semibold tabular-nums text-slate-900">{order.total}</td><td className="px-4 py-3"><span className="inline-flex items-center gap-2 text-xs font-semibold text-slate-700"><span className={cn('size-2 rounded-full', order.payment === 'Paid' ? 'bg-emerald-500' : 'bg-amber-400')} />{order.payment}</span></td><td className="px-4 py-3"><OrderStatus status={order.status} /></td></tr>)}</tbody></table></div>{filtered.length === 0 ? <div className="grid min-h-48 place-items-center border-t border-slate-100 p-6 text-center"><div><Search className="mx-auto size-5 text-slate-400" /><p className="mt-2 text-sm font-semibold text-slate-800">No matching orders</p><p className="mt-1 text-xs text-slate-500">Adjust the active view, status, or search query.</p></div></div> : null}</div>
      </div>
    </div>
