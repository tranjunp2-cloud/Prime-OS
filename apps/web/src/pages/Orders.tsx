    ...
    <div className="min-w-0 space-y-4">
        <div className="flex flex-col gap-2 rounded-xl border border-border bg-card p-3 md:flex-row md:items-center">
          ...
          <div className="grid shrink-0 grid-cols-2 gap-2 md:flex md:items-center">
            ...
            <Select value={activeView ?? 'none'} onValueChange={(value) => setActiveView(value === 'none' ? null : value as ActiveQueue)}><SelectTrigger className="h-10 w-full md:w-[190px]" aria-label="Select saved view"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">All operational views</SelectItem>{savedViews.map((savedView) => <SelectItem key={savedView.label} value={savedView.label}>{savedView.label === 'SLA at risk' ? 'Processing deadline at risk' : savedView.label} ({savedView.count})</SelectItem>)}</SelectContent></Select>
